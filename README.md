# 📈 Advanced Dual-Source News Sentiment Stock Screener Dashboard
...
Instead of traditional lagging price metrics, the screener scans real-time financial market news feeds (via **Alpha Vantage** general market feeds and **Yahoo Finance** ticker news), compiles tickers that have positive sentiment hits using custom positive keywords, and triggers an **on-demand, real-time LLM sentiment analysis** to assign high-accuracy investment scores.

---

## 🚀 Key Features

*   **📰 Dual-Source News Screener**: Scans general news feeds, pulls mentioned stocks, merges them with Yahoo Finance ticker news, compiles Keyword Sentiment Scores, and displays them alongside mention counts.
...
*   **💬 Mentioned Articles with Highlighter**: Lists all news headlines and summaries referencing the stock from both Yahoo and Alpha Vantage, utilizing an interactive regex highlighting filter to color-code matching positive keywords.
*   **🔄 Cache-Busting Refresh Control**: Clears internal yfinance and Alpha Vantage data caches and triggers a reload from the live endpoints, updating a "Last Data Sync" tracker.
*   **🛡️ Connector Status panel**: Monitor connection health for Yahoo Finance, Alpha Vantage (Vintage), and LLM API endpoints directly at the top of the dashboard. Shows detailed tooltips for error codes when keys are missing or rate limits are hit.
